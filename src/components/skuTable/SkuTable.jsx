...
const SkuTable = ({filter, chartId, height, width}) => {
  ...
  return (
    <div className="skuTableBox">
        <div className="chart" ref={chartDiv}/>
        <p onClick={()=>setGetData(true)} className="download">Press here to export the data</p>
        {getData === true && (
        <ExportJsonCsv headers={headers} items={skuData} fileTitle="sku-floship.csv">Download data as a csv file</ExportJsonCsv> 
        )}
    </div>
  );
};
